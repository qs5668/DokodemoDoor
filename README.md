...
- **运生物**：合成门之钥·实体（3h 档：紫晶×4 + 末影珍珠×4 + 恶魂之泪×1 → 1 把；两把同档合成一把高一档，档位 3/8/12/20/35/42/48h）配对，门即玩家与生物通用。**钥匙永不过期**，配对时门对开始计时，**到期自动解绑**（门方块保留，可重新配对）；GUI 实时显示剩余时限。腐竹可用 `/ddoor key entity <小时> [数量] [玩家]` 发放任意时长（0 = 永久门对）
...
- **命令**：`/ddoor gui|list|tp|rename|unlink|link|delete|key|stats|reload`（`key entity <小时> [数量] [玩家]` 可发放任意时限实体钥匙）
...
| 实体门 | 门之钥·实体配对的门玩家与生物通用，落点同样安全校验；七档门对限时，到期自动解绑 |
