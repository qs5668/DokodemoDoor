...
- **Spigot / CraftBukkit 支持**：新增 `TextAdapter` 平台抽象层，同一套业务代码编译为两个发行包——
  - `ddoor-1.0.2-paper.jar`：Paper / Leaves / Purpur，原生 Adventure 与 `teleportAsync`；
  - `ddoor-1.0.2-spigot.jar`：Spigot / CraftBukkit，shade 并重定位 Adventure（`top.midream.ddoor.libs.kyori`），Legacy §x 十六进制色渲染。
...
- Maven 双 profile（`-P paper` / `-P spigot`），build-helper 分平台源码目录 + shade 重定位。
